package android.slc.appbase.data.api.ds.callback;

import android.slc.appbase.R;
import android.slc.appbase.data.api.ApiConfig;
import android.slc.appbase.data.api.ds.callback.po.SlcEntityDs;
...
public abstract class SlcObserverDs<T> implements Observer<SlcEntityDs<T>> {
    protected SlcCallbackConfig mSlcCallbackConfig = SlcCallbackConfig.defSlcCallbackConfig;

    public SlcObserverDs() {

    }

    public SlcObserverDs(SlcCallbackConfig slcCallbackConfig) {
    ...
    @Override
    public void onNext(@NonNull SlcEntityDs<T> tSlcEntity) {
        onFinish();
    ...
    public void onError(@NotNull Throwable e) {
        SlcEntityDs<T> slcEntity = getErrorMsg(e);
    ...
    protected SlcEntityDs<T> getErrorMsg(Throwable e) {
        SlcEntityDs<T> slcEntity = null;
        if (e instanceof NullPointerException) {
            ...
            if (response != null) {
                ...
                if (errorBody != null) {
                    slcEntity = GsonUtils.fromJson(errorBody.charStream(), SlcEntityDs.class);
                }
            }
